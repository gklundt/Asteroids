public class GlideShipBehavior implements iShipBehavior {
    ...
    public MovementVector thrust(MovementVector current) {
        ...
        double x2 = ((double)current.speed + 1) * Math.cos(Math.toRadians(current.facing));
        double y2 = ((double)current.speed + 1) * Math.sin(Math.toRadians(current.facing));
        ...
        current.speed = (int)r > SHIPSPEED ? SHIPSPEED :(int)r;
